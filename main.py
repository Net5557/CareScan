...
def translate_class_to_thai(class_id):
    translations = {
            'Laceration': 'แผลฉีก',
            'Bruises': 'รอยฟกช้ำ',
            'Burns': 'แผลไฟไหม้',
            'Abrasions': 'แผลถลอก',
            'Cut': 'แผลตัด',
            'Ingrown-nails': 'เล็บขบ',
            'Stab-wound': 'แผลถูกแทง',
    }
    return translations.get(class_id, class_id)
...
async def predict(image: UploadFile = File(...)):
    ...
    for box in result.boxes:
        ...
        predictions.append({
            "class_id": translate_class_to_thai(class_id),
            "coordinates": cords,
            "confidence": conf
        })
